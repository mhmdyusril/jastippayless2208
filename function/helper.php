<?php 
	...
	define("BASE_URL", "http://localhost/KKP/");
	...
	function pagination($query, $data_per_halaman, $pagination, $url){
		...
		for($i = $mulaiPagination; $i<=$batasAkhirPagination; $i++){
			if ($pagination == $i){
				echo "<li><a class='active' href='".BASE_URL."$url&action=list&pagination=$i'>$i</a></li>";
			}else{
		...
		if ($pagination < $total_halaman) {
			$prev = $pagination + 1;
			echo "<li><a href='".BASE_URL."$url&pagination=$prev'>Next >></a></li>";
		}
		echo "</ul>";
	}
 ?>